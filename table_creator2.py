...
outputz.write("Transcript\tGene\tExonIntronSeq\tTranscriptSize\tStarts-Ends\tUniqID\tFlags\tSequence\n")# open output and print header
...
for transcript_name in UniqTranscripts: # for each uniq transcript:
    ...
    outputz.write(str(InputSubset['ReadName'].iloc[0]) + "\t" + str(Gene) + "\t" + str(EIitems) + "\t" + str(InputSubset['ReadSize'].iloc[0]) + "\t" + str(EIloci) + "\t" + str(InputSubset['ReadName'].iloc[0]) + "." + str(Gene) + "." + str(EIitems) + "\t" + str(Flags) + "\t" + str(reads_recorder[InputSubset['ReadName'].iloc[0]]) + "\n") # prints all important things to the output
