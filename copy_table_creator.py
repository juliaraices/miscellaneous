...
for transcript_name in UniqTranscripts: # for each uniq transcript:
    ...
    for transcript_instance in range(len(InputSubset)): # for each instace of that transcript
        ...
        if TranscriptData.empty: # if the new hash is empty we add things the easiest way ;)
            ...
            if "_" in NewGene: # if more than one gen comprises the already known genes in the transcript (their names are separated with _ )
                ...
                TranscriptData['Gene'].iloc[0] = TranscriptData['Gene'].iloc[0]+"_"+NewGene # just adds the new gene to the stored ones
            ...
            #TranscriptData['ExonIntronSeq'].iloc[0] = TranscriptData['ExonIntronSeq'].iloc[0]+","+NewItem
        #else:
            #TranscriptData['ExonIntronSeq'].iloc[0] = TranscriptData['ExonIntronSeq'].iloc[0]+","+NewItem
        # if Qstart > Qend
        ...
        #TrueStart = int(''.join(s for s in StartChr if s.isdigit()))+int(InputSubset['Qstart'].iloc[j]) #
        #TrueEnd = int(''.join(s for s in EndChr if s.isdigit()))+int(InputSubset['Qend'].iloc[j]) #
        SE = InputSubset['Tstart'].iloc[transcript_instance]+"-"+InputSubset['Tend'].iloc[transcript_instance] # adds the start and end of match
        #print(SE+"\n")
        if TranscriptData['Starts-Ends'].iloc[0] == "NA": # if there's no start-end set
            ...
            SplintersSE = TranscriptData['Starts-Ends'].iloc[0].split(",") # split starts and ends of trnascript
            ...
            for m in range(len(SplintersSE)): # for each item in the hash
                if (SplintersSE[m]==SE) and (SplintersI[m]==NewItem):
                    log.write("Same match, ("+str(NewItem)+") appeared more than once for read "+InputSubset['Tname'].iloc[j]+" in the same place ("+SE+").\n")
                    eita="y"
                    if flag == "":
                        flag = "b"
                    elif not "b" in flag:
                    ...
                    log.write("Same match, ("+str(NewItem)+") appeared more than once for read "+InputSubset['Tname'].iloc[j]+" in different places ("+SE+", and "+SplintersSE[m]+").\n")
                    eita="y"
                    if flag == "":
                        flag = "c"
                    elif not "c" in flag:
                    ...
                    log.write("Different matches, ("+str(NewItem)+" and "+str(SplintersI[m])+") appeared in the same place ("+SE+") for read "+InputSubset['Tname'].iloc[j]+".\n")
                    eita="y"
                    if flag == "":
                        flag = "d"
                    elif not "d" in flag:
                        flag = flag+",d"
            if eita != "y":
                TranscriptData['Starts-Ends'].iloc[0] = TranscriptData['Starts-Ends'].iloc[0]+","+SE
                TranscriptData['ExonIntronSeq'].iloc[0] = TranscriptData['ExonIntronSeq'].iloc[0]+","+str(NewItem)
    StarkEnds=TranscriptData['Starts-Ends'].iloc[0].split(",")
    StarkItems=TranscriptData['ExonIntronSeq'].iloc[0].split(",")
    ...
    for p in range(len(StarkEnds)):
        ...
        InputSubsetorery = [inicio, fim, tamanho, StarkItems[p]]
        Sansa = Sansa.append([InputSubsetorery], ignore_index=True)
    ...
    for q in range(len(Sansa)): # for item in sansa
        ...
        if q == 0: # if you are in the very first item
            StarkSE = str(NewSE) # just sets StarkSE as the NewSE
            StarkI = str(Sansa['item'].iloc[q]) # just sets the StarkI as the item in Sansa
