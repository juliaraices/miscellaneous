...
parser.add_argument("--fastaDB", default="/nfs/scistore03/vicosgrp/jraices/melanogaster/dmel_exon_intron.fasta", type=str, help='Fasta file used in blat. Default is the file with exons and introns in Drosophila melanogaster') # sets the default fasta file to be used in case it is not provided by the user
...
outputz.write("Transcript\tGene\tExonIntronSeq\tTranscriptSize\tTStarts-Ends\tEIStarts-Ends\tUniqID\tFlags\tSequence\n")# open output and print header
...
for transcript_name in UniqTranscripts: # for each uniq transcript:
    ...
    outputz.write(str(InputSubset['ReadName'].iloc[0]) + "\t" + str(Gene) + "\t" + str(EIitems) + "\t" + str(InputSubset['ReadSize'].iloc[0]) + "\t" + str(EIloci) + "\t" + str(QEIloci) + "\t" + str(InputSubset['ReadName'].iloc[0]) + "." + str(Gene) + "." + str(EIitems) + "\t" + str(Flags) + "\t" + str(reads_recorder[InputSubset['ReadName'].iloc[0]]) + "\n") # prints all important things to the output
